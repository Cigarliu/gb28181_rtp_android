...
namespace BXC {
    ...
    #define RTP_MAX_PACKET_BUFF 1300 // rtp传输时的最大包长
    ...
    public:
        ...
        explicit RtpSendPs(const char* rtpServerIp,int rtpServerPort,int localRtpPort, ConcurrentQueueA<std::shared_ptr<NALUData>>* naluQueue);
        RtpSendPs() = delete;
        ~RtpSendPs();
    public:
        void start();
        void stop();
        void setNaluQueue(ConcurrentQueueA<std::shared_ptr<NALUData>>* naluQueue);
    private:
    ...
    private:
        ...
        // 队列用于接收NALUData
        ConcurrentQueueA<std::shared_ptr<NALUData>>* mNaluQueue;
    };
